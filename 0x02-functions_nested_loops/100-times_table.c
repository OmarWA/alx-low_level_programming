...
int product = 0, product_hundreds = 0, product_tens = 0, product_units = 0;
if (!((n > 15) || (n < 0)))
{
while (number <= n)
{
...
product_hundreds = (product - product_tens - product_units) / 100;
if (iterator == 0)
goto SKIP;
if (product_tens == 0 || product_hundreds == 0)
_putchar(' ');
else
_putchar('0' + product_hundreds);
